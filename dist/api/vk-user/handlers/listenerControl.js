...
export async function handleReloadAccount(fastify, request, reply) {
    try {
        ...
        if (!account_id || !user_id) {
            fastify.log.warn(`⚠️  Неверный payload для /api/internal/vk-user/listener/reload-account: ${JSON.stringify(body)}`);
        ...
        if (!baseUrl) {
            fastify.log.error("❌ VK_SERVICE_URL не установлена в переменных окружения");
        ...
        if (!response.ok) {
            fastify.log.error(`❌ Ошибка при вызове VK Service reload-account: ${response.status}`);
