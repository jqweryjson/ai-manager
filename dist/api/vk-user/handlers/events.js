...
import { publishVkSendMessage } from "../../../core/queue-publisher.js";
import { canSendMessage } from "../../../core/vk-account-postgres.js";
/**
 * POST /internal/vk-user/events - обработка событий от VK Listener
 * Внутренний endpoint, без аутентификации (доступен только внутри Docker сети)
 */
export async function handleVkEvent(fastify, request, reply) {
    try {
        ...
        if (event.is_outgoing === true) {
            console.log(`⏭️  Пропущено исходящее сообщение для ${event.account_id}/${event.peer_id}`);
        ...
        // Проверяем, можно ли отправить сообщение (rate limiting)
        const canSend = await canSendMessage(event.account_id, event.peer_id);
        if (!canSend) {
            console.log(`⏸️  Чат ${event.account_id}/${event.peer_id} заблокирован (rate limit). Пропускаем генерацию ответа.`);
            return {
                success: true,
                skipped: true,
                reason: "rate_limit",
                message: "Chat is blocked by rate limit",
            };
        }
        console.log(`🎯 Обработка события от VK Listener:`);
        ...
        // Публикуем задачу на отправку в RabbitMQ
        try {
            const published = await publishVkSendMessage({
                type: "send_message",
                integration: "vk",
        ...
        console.error("❌ Ошибка обработки VK события:", error);
