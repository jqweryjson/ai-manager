...
import { insertDocuments, listUserDocuments, deleteUserDocument, } from "../core/milvus.js";
import { getWorkspace, ensureDefaultWorkspace, } from "../core/workspace-postgres.js";
export async function ingestRoutes(fastify) {
    await fastify.register(multipart);
    fastify.post("/ingest", { preHandler: fastify.authenticate }, async (request, reply) => {
        const req = request;
        ...
        const workspaceId = workspaceIdField &&
        ...
        try {
            ...
            fastify.log.info(`🔄 Начало генерации embeddings для ${texts.length} чанков...`);
            ...
            // Insert into Milvus
            fastify.log.info(`💾 Начало записи в Milvus: ${documents.length} документов...`);
            ...
            fastify.log.info(`📋 Итого: ${chunks.length} чанков, ${documents.length} векторов`);
            return {
                doc_id: docId,
    ...
    fastify.delete("/documents/:docId", { preHandler: fastify.authenticate }, async (request, reply) => {
        const req = request;
