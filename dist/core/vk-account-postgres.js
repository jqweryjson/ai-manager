 ...
 */
export async function updateVkAccountAuthData(accountId, userId, params) {
...
export async function upsertUserVkAccount(params) {
    ...
    if (!primary) {
        return createVkAccount(params.userId, params.accessToken, params.refreshToken, params.userIdVk, params.expiresAt);
    }
    ...
    if (accounts.length > 1) {
        await withPostgres(async (client) => {
         ...
         WHERE user_id = $1 AND id <> $2`, [params.userId, primary.id]);
        });
    }
