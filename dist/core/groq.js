...
function stripThink(answer, includeThink) {
    if (includeThink)
        return answer;
    return answer.replace(/<think>[\s\S]*?<\/think>/gi, "").trim();
}
export async function callGroqLLM(question, context, customSystemPrompt, model = process.env.GROQ_MODEL || "", options) {
    let systemPrompt;
...
${context}`;
    ...
    try {
        ...
        const raw = completion.choices[0]?.message?.content || "Ошибка получения ответа";
        ...
        throw new Error(`Ошибка вызова LLM: ${error instanceof Error ? error.message : "Неизвестная ошибка"}`);
    }
}
