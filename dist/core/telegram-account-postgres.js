 ...
 */
export async function createTelegramAccount(userId, apiId, apiHash, phone, phoneCodeHash, sessionString) {
    ...
    return await withTransaction(async (client) => {
        const result = await client.query(`INSERT INTO telegram_accounts 
 ...
 */
export async function getTelegramAccount(accountId, userId) {
    return await withPostgres(async (client) => {
        const result = await client.query(`SELECT * FROM telegram_accounts 
 ...
 */
export async function updateTelegramAccountSession(accountId, userId, sessionString, status) {
    const encryptedSession = encrypt(sessionString, BACKEND_SECRET);
    await withPostgres(async (client) => {
        await client.query(`UPDATE telegram_accounts 
       SET session = $1, status = $2, updated_at = NOW()
       WHERE id = $3 AND user_id = $4`, [encryptedSession, status, accountId, userId]);
    });
}
/**
 * Обновляет статус аккаунта
 */
export async function updateTelegramAccountStatus(accountId, userId, status, floodWaitUntil) {
    await withPostgres(async (client) => {
        await client.query(`UPDATE telegram_accounts 
       SET status = $1, flood_wait_until = $2, updated_at = NOW()
       WHERE id = $3 AND user_id = $4`, [status, floodWaitUntil || null, accountId, userId]);
    });
}
/**
 * Получает расшифрованные данные аккаунта
 */
export function decryptTelegramAccount(account) {
 ...
 * Удаляет аккаунт
 */
export async function deleteTelegramAccount(accountId, userId) {
    await withPostgres(async (client) => {
        await client.query(`DELETE FROM telegram_accounts 
       WHERE id = $1 AND user_id = $2`, [accountId, userId]);
    });
}
export async function listSubscriptions(accountId, userId) {
    ...
    return await withPostgres(async (client) => {
        const res = await client.query(`SELECT * FROM telegram_subscriptions
...
export async function upsertSubscriptions(accountId, userId, items) {
    const account = await getTelegramAccount(accountId, userId);
    if (!account) {
        throw new Error("Account not found");
    }
