...
export async function migrateUsersFromRedis() {
    ...
    for (const key of userKeys) {
        try {
            ...
            if (existingUser) {
                console.log(`⏭️  Пользователь ${user.email} уже существует в PostgreSQL`);
                continue;
            }
            // Вставляем пользователя в PostgreSQL
            await withPostgres(async (client) => {
                await client.query(`INSERT INTO users (id, email, name, picture, created_at) 
           VALUES ($1, $2, $3, $4, $5)`, [
                    user.id,
                    user.email,
                    user.name,
