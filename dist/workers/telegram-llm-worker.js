...
async function handleMessage(channel, msg) {
    ...
    try {
        ...
        console.log(`📥 [telegram-llm-worker] Получено событие: ${payload.account_id}/${payload.peer_id}`);
        // Базовая валидация
        if (!payload.message_text ||
            payload.message_text.trim().length === 0 ||
            !payload.workspace_id ||
            !payload.role_id) {
            console.log(`⚠️  [telegram-llm-worker] Некорректное событие (нет текста или workspace/role), пропускаем: ${payload.account_id}/${payload.peer_id}`);
            channel.ack(msg);
            return;
        }
        ...
        if (!canSend) {
            console.log(`⏸️  [telegram-llm-worker] Чат ${payload.account_id}/${payload.peer_id} заблокирован (FLOOD_WAIT). Пропускаем генерацию ответа.`);
            channel.ack(msg);
            return;
        }
        // Генерация ответа через chat API
        const chatStart = Date.now();
        const chatResponse = await generateChatResponse(payload.message_text, payload.user_id, payload.workspace_id, payload.role_id);
        const chatTime = Date.now() - chatStart;
        if (!chatResponse.success) {
            console.error(`❌ [telegram-llm-worker] Ошибка генерации ответа: ${chatResponse.error || "unknown"}`);
            channel.ack(msg);
            return;
        }
        console.log(`✅ [telegram-llm-worker] Ответ сгенерирован за ${chatTime}ms (${chatResponse.answer.length} символов)`);
        ...
        if (published) {
            console.log(`📤 [telegram-llm-worker] Задача на отправку опубликована для ${payload.account_id}/${payload.peer_id}`);
        }
        else {
            console.error(`❌ [telegram-llm-worker] Не удалось опубликовать задачу на отправку для ${payload.account_id}/${payload.peer_id}`);
        }
        channel.ack(msg);
    }
    catch (error) {
...
async function startTelegramLLMWorker() {
    ...
    await channelWrapper.addSetup(async (channel) => {
        ...
        console.log(`✅ [telegram-llm-worker] Подписан на очередь ${queueName} (exchange: ${TELEGRAM_CONFIG.exchange})`);
    });
}
...
//# sourceMappingURL=telegram-llm-worker.js.map