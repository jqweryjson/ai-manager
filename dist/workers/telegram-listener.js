...
class TelegramListenerManager {
    ...
    clientManager = new TelegramClientManager();
    ...
    async initialize() {
        try {
            ...
            console.log(`✅ Инициализировано ${this.clients.size} слушателей Telegram`);
        }
        catch (error) {
     ...
     */
    async startListening(accountId, userId) {
        if (this.clients.has(accountId)) {
            console.log(`⚠️  Слушатель для ${accountId} уже запущен`);
            return;
        }
        try {
            const clientInfo = await this.clientManager.createClient(accountId, userId);
            if (!clientInfo) {
                return;
            }
            ...
            client.addEventHandler(async (event) => {
                try {
                    ...
                    console.error(`❌ Ошибка обработки сообщения для ${accountId}:`, error);
                }
            }, this.clientManager.getNewMessageConfig());
            this.clients.set(accountId, clientInfo);
            console.log(`✅ Слушатель запущен для ${accountId} (${clientInfo.phone})`);
        }
        catch (error) {
            console.error(`❌ Не удалось запустить слушатель для ${accountId}:`, error);
     ...
     */
    async handleNewMessage(accountId, userId, messageEvent) {
        ...
        const event = await this.messageProcessor.buildEvent(accountId, userId, peer_id, message);
        if (!event) {
            return;
        }
    ...
    async shutdown() {
        ...
        const disconnectPromises = Array.from(this.clients.entries()).map(async ([accountId, info]) => {
            try {
