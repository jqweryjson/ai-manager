...
import type { TelegramSendMessage } from "../core/queue-publisher.js";
...
async function handleMessage(
  ...
  try {
    ...
    const payload: TelegramSendMessage = JSON.parse(content);

    console.log(
      `📥 [telegram-sender] Получена задача: ${payload.account_id}/${payload.peer_id}`
    );
    ...
    if (!canSend) {
      console.log(
        `⏸️  [telegram-sender] Чат ${payload.account_id}/${payload.peer_id} ещё заблокирован (FLOOD_WAIT). Пропускаем задачу.`
      );
      channel.ack(msg);
      return;
    }

    const result: SendTelegramMessageResult = await sendTelegramMessage(
      payload.account_id,
      payload.user_id,
      payload.peer_id,
      payload.peer_type,
    ...
    if (result.success) {
      ...
      await updateNextAllowedAt(
        payload.account_id,
        payload.peer_id,
        BASE_DELAY_SECONDS
      );
      console.log(
        `✅ [telegram-sender] Сообщение отправлено, next_allowed_at обновлён на +${BASE_DELAY_SECONDS} сек`
      );
      channel.ack(msg);
      return;
    }
    ...
    if (result.floodWaitSeconds && result.floodWaitSeconds > 0) {
      await updateNextAllowedAt(
        payload.account_id,
        payload.peer_id,
        result.floodWaitSeconds
      );
      console.log(
        `⏸️  [telegram-sender] FLOOD_WAIT для ${payload.account_id}/${payload.peer_id}: ${result.floodWaitSeconds} сек. Обновлён next_allowed_at.`
      );
    ...
    // Другая ошибка
    console.error(
      `❌ [telegram-sender] Ошибка отправки сообщения для ${payload.account_id}/${payload.peer_id}: ${
...
async function startTelegramSender(): Promise<void> {
  ...
  await channelWrapper.addSetup(async (channel: ConfirmChannel) => {
    ...
    console.log(
      `✅ [telegram-sender] Подписан на очередь ${queueName} (exchange: ${TELEGRAM_CONFIG.exchange})`
    );
  });
}
