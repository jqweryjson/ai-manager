...
export async function migrateWorkspacesFromRedis(): Promise<void> {
  ...
  for (const key of workspaceKeys) {
    try {
      ...
      const workspace: RedisWorkspace = JSON.parse(workspaceData);
      ...
      if (existingWorkspace) {
        console.log(
          `⏭️  Workspace ${workspace.name} уже существует в PostgreSQL`
        );
        continue;
      }
  ...
  console.log(
    `🎉 Миграция workspace завершена! Успешно: ${migrated}, Ошибок: ${errors}`
  );
}
