...
import { getAllQueueConfigs, type QueueConfig } from "./queue-config.js";
 ...
 */
export async function getRabbitMQConnection(): Promise<AmqpConnectionManager> {
  if (connection) {
    return connection;
  }
  ...
  connection.on("connectFailed", (params: { err?: Error }) => {
 ...
 */
export async function getRabbitMQChannel(): Promise<ChannelWrapper> {
  if (channelWrapper) {
    ...
    setup: async (channel: ConfirmChannel) => {
      ...
      for (const config of configs) {
        ...
        await channel.bindQueue(
          config.queues.events,
          config.exchange,
          config.queues.events
        );

        await channel.bindQueue(
          config.queues.sendMessage,
          config.exchange,
          config.queues.sendMessage
        );

        console.log(
          `✅ Настроены очереди для интеграции: ${config.integration}`
        );
      }
