 ...
 */
function parseRateLimit(error: any): number | null {
  const errorMessage = error?.message || String(error);

  // VK API error_code 6 = Too many requests
  if (
    errorMessage.includes("error_code: 6") ||
...
export async function sendVkMessage(
  ...
  userId: string,
  peerId: string,
  peerType: "user" | "chat" | "group",
  text: string
): Promise<SendVkMessageResult> {
  ...
  try {
    ...
    if (
      error?.message?.includes("error_code: 5") ||
