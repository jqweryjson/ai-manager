 ...
 */
export async function handleVkEvent(
  fastify: FastifyInstance,
  request: any,
  reply: FastifyReply
) {
  try {
    const event: VkEvent = request.body;
    ...
    if (event.is_outgoing === true) {
      console.log(
        `⏭️  Пропущено исходящее сообщение для ${event.account_id}/${event.peer_id}`
      );
    ...
    if (!canSend) {
      console.log(
        `⏸️  Чат ${event.account_id}/${event.peer_id} заблокирован (rate limit). Пропускаем генерацию ответа.`
      );
      return {
        success: true,
        skipped: true,
