...
export async function handleStart(
  ...
  try {
    ...
    fastify.log.info(
      `📱 Начало подключения VK для пользователя ${request.userId}`
    );
...
export async function handleCallback(
  ...
  try {
    ...
    if (!userId) {
      return reply.redirect(
        `${FRONTEND_URL}/app/integrations?error=missing_state`
      );
    }
    ...
    if (!appId || !appSecret || !redirectUri) {
      fastify.log.error(
        "VK_APP_ID, VK_APP_SECRET or VK_REDIRECT_URI is not set"
      );
      return reply.redirect(
        `${FRONTEND_URL}/app/integrations?error=server_config`
      );
    }
    ...
    try {
      ...
      await fetch(
        `${backendUrl}/api/internal/vk-user/listener/reload-account`,
        {
          method: "POST",
          headers: {
      ...
      fastify.log.info(
        `📡 Отправлен запрос reload-account для VK Service: ${account.id}`
      );
    } catch (notifyError) {
      fastify.log.error(
        `⚠️  Не удалось отправить reload-account для VK Service: ${notifyError}`
      );
    }

    return reply.redirect(
      `${FRONTEND_URL}/app/integrations?success=vk_connected&account_id=${account.id}`
    );
  } catch (error: any) {
    fastify.log.error(`❌ Ошибка OAuth callback: ${error}`);
    return reply.redirect(
      `${FRONTEND_URL}/app/integrations?error=oauth_failed`
    );
  }
}
