  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (body.role_id) {
        assistantRole = await withPostgres(async client => {
          const result = await client.query(
            "SELECT id, name, prompt FROM roles WHERE id = $1 AND user_id = $2",
            [body.role_id, req.userId]
          );
      ...
      try {
        ...
        const milvusRes = await searchSimilar(
          queryVector,
          req.userId,
          body.workspace_id,
          RAG_TOP_K
        ...
        fastify.log.info(
          `✅ Найдено кандидатов: ${milvusRes.length} за ${rTime}ms`
        );

        // Приведение результатов к унифицированному формату
        const retrieved = (milvusRes || []).map((r: any) => ({
          id: r.id,
          docId: r.doc_id,
          chunkId: r.chunk_id,
          text: r.text,
        ...
        fastify.log.info(
          `🧱 Контекст: ${built.usage.chunks} чанков, ${built.usage.docs} доков, ${built.usage.chars} симв., за ${ctxTime}ms`
        );
        ...
        const llmResponse = await callGroqLLM(
          body.question,
          contextString,
          assistantRole?.prompt,
          undefined,
          { includeThink: body.show_think }
        );
